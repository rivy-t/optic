...
test({
	...
	fn() {
		...
		assertMatch(stringify(new Date('2020-06-17T03:24:00')), /2020-06-1[6,7,8]T\d\d:\d\d:00.000Z/);
	},
});
...
test({
	...
	fn() {
		...
		assertEquals(stringify([null, undefined, true, 9n, NaN]), `[null,"undefined",true,"9","NaN"]`);
	},
});
...
test({
	...
	fn() {
		assertEquals(stringify(new Error('hello'), { suppressErrorStack: true }), `"Error: hello"`);
		assertMatch(
...
test({
	...
	fn() {
		...
		const obj = { first: { subFirst: map }, second: { subSecond: SEPT_24_2020 } };
		assertMatch(
...
test({
	...
	fn() {
		...
		const deepRecursiveObj = { some: 'prop', deeply: { recursive: { obj: {} } } };
