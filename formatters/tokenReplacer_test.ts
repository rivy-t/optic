...
import { assert, assertEquals, assertMatch, assertThrows, test } from '../test_deps.ts';
...
test({
	...
	fn() {
		...
		assertEquals(new TokenReplacer().formatString, '{dateTime} {level} {msg} {metadata}');
	},
});
...
test({
	...
	fn() {
		let output = new TokenReplacer().withFormat('{dateTime}').withDateTimeFormat('YYYY').format(lr);
		assertEquals(output, '2020');
		output = new TokenReplacer()
			.withFormat('{dateTime}')
			.withDateTimeFormat(() => 'from fn')
			.format(lr);
		assertEquals(output, 'from fn');
		output = new TokenReplacer()
			.withFormat('{dateTime}')
			.withDateTimeFormat({ format: () => 'from class' })
			.format(lr);
...
test({
	...
	fn() {
		...
		assertEquals(tr.format(getMsgLr({ a: 6, b: 'hello' })), '{"a":6,"b":"hello"}');
...
test({
	...
	fn() {
		...
		assertEquals(tr.format(getMetadataLr([[1, 2, 3], 'b', err])), '[1,2,3] b ' + err.stack);
	},
});
...
test({
	...
	fn() {
		...
		lr.level = Level.Debug;
		assertEquals(tr.format(lr), gray('2020-06-17T02:24:00.000Z Debug    Log Message The metadata'));
	},
});
...
test({
	...
	fn() {
		...
		lr.level = 99;
		assertEquals(tr.format(lr), 'Log Message default');
	},
});
