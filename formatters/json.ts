...
export class JsonFormatter implements Formatter<string> {
	...
	format(logRecord: LogRecord): string {
		...
		for (const field of this.#fields) {
			if (field === 'dateTime') {
				output += '"dateTime":' +
					stringify(logRecord.dateTime, { dateTimeFormatter: this.#dateTimeFormatter }) + ',';
			} else if (field === 'level') {
		...
		if (this.#indent !== 0) {
			output = stringify(JSON.parse(output), { indent: this.#indent }).replaceAll('\\n', '\n');
		}
		return output;
	}
   ...
   */
	...
	withDateTimeFormat(dtf: DateTimeFormatterFn | DateTimeFormatter | string): this {
