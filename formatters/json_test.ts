...
test({
	...
	fn() {
		...
		const jfMsgDateTime = new JsonFormatter().withFields(['dateTime', 'msg', 'logger']);
		assertEquals(jfMsg.format(lr), '{"msg":{"a":6,"b":"hello"}}');
		assertEquals(jfDateTime.format(lr), '{"dateTime":"2020-06-17T02:24:00.000Z"}');
		assertEquals(jfLevel.format(lr), '{"level":"Debug"}');
		assertEquals(jfMetadata.format(lr), '{"metadata":[true,"undefined","there"]}');
		assertEquals(
