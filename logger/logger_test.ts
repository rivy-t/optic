...
import type { Filter, LogMeta, LogRecord, Monitor, Stream, Transformer } from '../types.ts';
...
test({
	...
	fn() {
		assertEquals(new Logger().addStream(new TestStream()).minLogLevel(), Level.Debug);
	},
});
...
test({
	...
	fn() {
		...
		assertEquals(testStream.meta?.minLogLevelFrom, 'from command line argument');
	},
});
...
test({
	...
	fn() {
		...
		new Logger().addStream(testStream).addMonitor(testMonitor).addFilter(testFilter).addTransformer(
			testTransformer,
		);
		dispatchEvent(new Event('unload'));
		assertEquals(testStream.functionsCalled, ['setup', 'logHeader', 'logFooter', 'destroy']);
...
test({
	...
	fn() {
		const testStream = new TestStream();
		const logger = new Logger().addStream(testStream).withMinLogLevel(Level.Info);
...
test({
	...
	fn() {
		...
		const resolvedMsg: string | undefined = logger.debug(() => 'resolved hello');
...
test({
	...
	fn() {
		...
		const logger = new Logger().addStream(new TestStream()).addMonitor(testMonitor1).addMonitor(
			testMonitor2,
		);
...
test({
	...
	fn() {
		const testMonitor = new TestMonitor();
		new Logger().addStream(new TestStream()).addMonitor(testMonitor).removeMonitor(testMonitor);
		assertEquals(testMonitor.functionsCalled, ['setup', 'destroy']);
	},
});
...
test({
	...
	fn() {
		const testFilter = new TestFilter();
		new Logger().addStream(new TestStream()).addFilter(testFilter).removeFilter(testFilter);
		assertEquals(testFilter.functionsCalled, ['setup', 'destroy']);
	},
});
...
test({
	...
	fn() {
		const testTransformer = new TestTransformer();
		new Logger().addStream(new TestStream()).addTransformer(testTransformer).removeTransformer(
			testTransformer,
		);
		assertEquals(testTransformer.functionsCalled, ['setup', 'destroy']);
	},
...
test({
	...
	fn() {
		...
		const logger = new Logger()
			.addStream(testStream1)
			.addStream(testStream2)
			.addFilter(filter1)
			.addFilter(filter2);
		...
		assertEquals(testStream1.functionsCalled, ['setup', 'logHeader', 'handle']);
		assertEquals(testStream2.functionsCalled, ['setup', 'logHeader', 'handle', 'handle']);
		...
		assertEquals(testStream1.functionsCalled, ['setup', 'logHeader', 'handle', 'handle']);
		assertEquals(testStream2.functionsCalled, ['setup', 'logHeader', 'handle', 'handle', 'handle']);
		assertEquals(filter1.filterCount, 4);
	...
	fn() {
		...
		class TestTransformer implements Transformer {
			...
			transform(stream: Stream, logRecord: LogRecord): LogRecord {
				...
				if (stream === testStream1 && (logRecord.msg as string).indexOf('transform') > -1) {
					this.replacements++;
		...
		assertEquals(testStream1.meta?.streamStats.get(testStream1)?.transformed, 1);
		assertEquals(testStream2.meta?.streamStats.get(testStream2)?.transformed, 0);
	},
});
...
test({
	...
	fn() {
		...
		assert(new Date().getTime() - testStream.logRecords[0].dateTime.getTime() < 10);
	},
});
...
test({
	...
	fn() {
		const testStream = new TestStream();
		const output = new Logger().addStream(testStream).warn({ a: 'b' }, [{ c: 'd' }]);
...
test({
	...
	fn() {
		...
		assertEquals(meta?.streamStats.get(testStream)?.handled.get(Level.Error), 3);
		assertEquals(meta?.streamStats.get(testStream)?.handled.get(Level.Warn), 2);
		assertEquals(meta?.streamStats.get(testStream)?.handled.get(Level.Info), 3);
		assertEquals(meta?.streamStats.get(testStream)?.handled.get(Level.Trace), undefined);
	},
});
