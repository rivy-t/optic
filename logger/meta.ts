...
export class LogMetaImpl implements LogMeta {
	...
	streamStats: Map<
		Stream,
		{ handled: Map<number, number>; filtered: number; transformed: number; duplicated: number }
	> = new Map();
	...
	toRecord(stream: Stream): Record<string, unknown> {
		...
		if (streamStats) {
			record.logRecordsHandled = Array
				.from(streamStats.handled.keys())
				.map((k) => levelToName(k) + ': ' + streamStats.handled.get(k))
				.join(', ');
