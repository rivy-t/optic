...
export class Logger {
   ...
   */
	...
	private logToStreams<T>(
		...
		let logRecord: LogRecord = new ImmutableLogRecord(resolvedMsg, metadata, level, this.#name);
			...
			if (!skip) {
				if (this.#transformers.length > 0) {
					...
					for (let j = 0; !skip && j < this.#transformers.length; j++) {
						...
						thisLogRecord = this.#transformers[j].transform(stream, thisLogRecord);
	...
	private registerStreamHandlingOfLogRecord(stream: Stream, level: number): void {
   ...
   */
	...
	warn<T>(msg: (T extends AnyFunction ? never : T), ...metadata: unknown[]): T;
	warn<T>(
		msg: () => T | (T extends AnyFunction ? never : T),
		...metadata: unknown[]
	): T | undefined {
   ...
   */
	...
	critical<T>(msg: (T extends AnyFunction ? never : T), ...metadata: unknown[]): T;
	critical<T>(
		msg: () => T | (T extends AnyFunction ? never : T),
		...metadata: unknown[]
	): T | undefined {
   ...
   */
	...
	log<T>(level: Level, msg: (T extends AnyFunction ? never : T), ...metadata: unknown[]): T;
	log<T>(
		level: Level,
		msg: () => T | (T extends AnyFunction ? never : T),
		...metadata: unknown[]
