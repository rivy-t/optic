...
import { assert, assertEquals, assertMatch, assertStringIncludes, test } from '../test_deps.ts';
...
test({
	...
	fn() {
		const baseStream = newBaseStream().withFormat(new AlternativeMsgPassThrough());
		baseStream.handle(logRec('hello', Level.Debug));
...
test({
	...
	fn() {
		...
		assertStringIncludes((baseStream.logs[0] as string), 'Logging session initialized');
	},
});
...
test({
	...
	fn() {
		...
		const meta = logMeta();
		meta.streamStats.set(testStream, {
		...
		assertStringIncludes((testStream.logs[0] as string), 'Logging session complete.  Duration');
	},
});
