...
import { LogFileInitStrategy, LogFileRetentionPolicy, RotationStrategy } from './types.ts';
...
function logRec(msg: string, level: Level): LogRecord {
	return { msg: msg, dateTime: new Date(), metadata: [], level: level, logger: 'default' };
}
...
test({
	name: 'LogFileInitStrategy of append should retain existing log file if file exists',
	fn() {
...
test({
	...
	fn() {
		...
		const fs = new FileStream(LOG_FILE).withLogFileRotation(rs).withLogFileInitMode('mustNotExist');
		fs.setup();
...
test({
	...
	fn() {
		const fs = new FileStream(LOG_FILE).withLogHeader(false).withLogFooter(false);
		fs.setup();
...
test({
	...
	fn() {
		...
		assert(handled);
		assertEquals(Deno.statSync(LOG_FILE).size, 12, 'Message immediately written to file');
		fs.destroy();
		Deno.removeSync(LOG_FILE);
	},
});
...
test({
	...
	fn() {
		...
		assertEquals(DECODER.decode(rs.rotateLogMessage as Uint8Array), 'second message\n');
		assertEquals(readFile(LOG_FILE + '.1'), 'first message\n');
		assertEquals(fs.getBuffer().buffered(), 'second message\n'.length);
		assertEquals(Deno.statSync(LOG_FILE).size, 0, 'Second message still buffered');
		fs.destroy();
		Deno.removeSync(LOG_FILE);
