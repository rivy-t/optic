...
export class DateTimeRotationStrategy implements RotationStrategy {
	...
	#filenameFormatter: ((filename: string, refDate: Date) => string) | null = null;

	constructor(interval: number, period: Periods) {
		if (interval < 1) {
			throw new ValidationError('DateTime rotation interval cannot be less than 1');
		}
		...
		if (initStrategy === 'append') {
			...
			if (logFiles.length > 0) {
				throw new IllegalStateError(
					'Found log file(s) which must not exist: ' + logFiles.toString(),
				);
			}
		}
	}
	...
	private handleLogFileRetention(filename: string) {
		...
		if (this.#logFileRetentionPolicy.type === 'files') {
			if (logFiles.length > this.#logFileRetentionPolicy.quantity) {
				...
				const sortedKeys = [...mtimeMap.keys()].sort((a, b) => b.getTime() - a.getTime());

				// Delete those beyond retention quantity index in sorted array
				for (let i = this.#logFileRetentionPolicy.quantity; i < logFiles.length; i++) {
   ...
   */
	...
	private setEndOfIntervalPeriod(): void {
		if (this.#period == 'days') {
			this.#endOfIntervalPeriod.setDate(this.#endOfIntervalPeriod.getDate() + this.#interval);
			this.#endOfIntervalPeriod.setHours(0, 0, 0, 0);
		} else if (this.#period == 'hours') {
			this.#endOfIntervalPeriod.setHours(this.#endOfIntervalPeriod.getHours() + this.#interval);
			this.#endOfIntervalPeriod.setMinutes(0, 0, 0);
		} else {
			this.#endOfIntervalPeriod.setMinutes(this.#endOfIntervalPeriod.getMinutes() + this.#interval);
   ...
   */
	withFilenameFormatter(formatter: (filename: string, refDate: Date) => string): this {
   ...
   */
	withLogFileRetentionPolicy(logFileRetentionPolicy: LogFileRetentionPolicy): this {
