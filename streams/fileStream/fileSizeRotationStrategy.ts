...
import type { LogFileInitStrategy, LogFileRetentionPolicy, RotationStrategy } from './types.ts';
	...
	initLogs(filename: string, initStrategy: LogFileInitStrategy): void {
		...
		if (initStrategy === 'append') {
			...
			if (this.#logFileRetentionPolicy.type === 'files') {
					...
					if (exists(filename + '.' + i)) {
						if (initStrategy === 'mustNotExist') {
							throw new IllegalStateError(
								'Found existing log file which must not exist: ' + filename + '.' + i,
							);
						} else {
				...
				const logFiles = getLogFilesInDir(filename, matchesFilePattern);
		...
		for (const logFile of logFiles) {
			...
			if (matched?.[1]) {
				if (this.#logFileRetentionPolicy.type === 'files') {
					...
					if (statInfo && statInfo < this.#logFileRetentionPolicy.oldestRetentionDate().getTime()) {
						Deno.removeSync(logFile);
					}
				}
			}
