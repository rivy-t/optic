...
function getDateRotatedFilename(file: string, d: Date): string {
	return file + '_' + d.getFullYear() + '.' + twoDig(d.getMonth() + 1) + '.' + twoDig(d.getDate());
}
...
test({
	name: 'Append strategy init logs with same interval as last run will not rotate',
	async fn() {
...
test({
	name: 'Append strategy, init logs, old interval outwith retention deletes log files',
	...
	async fn() {
		...
		const rs = new DateTimeRotationStrategy(1, 'days').withLogFileRetentionPolicy(of(14).days());
		assert(exists(LOG_FILE));
...
test({
	...
	async fn() {
		await createFile(LOG_FILE);
		const rs = new DateTimeRotationStrategy(1, 'days').withLogFileRetentionPolicy(of(7).days());
		const d = new Date();
		rs.rotate(LOG_FILE);
		assert(!exists(LOG_FILE));
...
test({
	name: 'rotate days strategy, date/time retention with retained and non retained logs',
	async fn() {
		...
		const rs = new DateTimeRotationStrategy(1, 'days').withLogFileRetentionPolicy(of(3).days());
		const d = new Date();
...
test({
	...
	async fn() {
		...
		const rs = new DateTimeRotationStrategy(1, 'hours').withLogFileRetentionPolicy(of(3).files());
		const d = new Date();
