...
async function setMTime(file: string, date: Date): Promise<void> {
	...
	const p = Deno.run({ cmd: ['touch', '-m', '-t', modTime, file] });
	await p.status();
	p.close();
}
...
test({
	name: 'FileSizeRotationStrategy: init append will set current file size to 0 for new log file',
	fn() {
		const rs = every(100).bytes();
		rs.initLogs(LOG_FILE, 'append');
...
test({
	name: 'FileSizeRotationStrategy: mustNotExist strategy will throw for in scope log file',
	async fn() {
...
test({
	name: 'FileSizeRotationStrategy: overwrite strategy will delete for in scope log file',
	async fn() {
...
test({
	name: 'FileSizeRotationStrategy: Log file with days date/time retention policy',
	ignore: Deno.build.os === 'windows',
	async fn() {
		const d = new Date();
...
test({
	name: 'FileSizeRotationStrategy: Log file with hours date/time retention policy',
	ignore: Deno.build.os === 'windows',
	async fn() {
		const d = new Date();
...
test({
	name: 'FileSizeRotationStrategy: Log file with minutes date/time retention policy',
	ignore: Deno.build.os === 'windows',
	async fn() {
		const d = new Date();
...
test({
	name: 'FileSizeRotationStrategy: file based rotation will rotate original log file',
	async fn() {
		await createFile(LOG_FILE);
...
test({
	name: 'FileSizeRotationStrategy: file based rotation will rotate all in scope log files',
	fn() {
...
test({
	name: 'FileSizeRotationStrategy: dateTime based rotation will rotate all in scope log files',
	ignore: Deno.build.os === 'windows',
	async fn() {
		Deno.writeFileSync(LOG_FILE, encoder.encode('orig'));
		Deno.writeFileSync(LOG_FILE + '.1', encoder.encode('1'));
...
test({
	name: 'FileSizeRotationStrategy: dateTime based rotation will rotate the only in scope log files',
	ignore: Deno.build.os === 'windows',
	fn() {
		Deno.writeFileSync(LOG_FILE, encoder.encode('orig'));
