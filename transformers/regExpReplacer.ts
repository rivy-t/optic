 ...
 */
export function alphaNumericReplacer(fullMatch: string, ...args: unknown[]): string {
 ...
 * See also https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/replace#Specifying_a_function_as_a_parameter
 */
export function nonWhitespaceReplacer(fullMatch: string, ...args: unknown[]): string {
	if (args.length === 2) { // e.g. no groups, obfuscate all of match
		...
		for (let i = 0; i < args.length - 2; i++) {
			const argValAsString = args[i] as string;
			returnVal = returnVal.replace(argValAsString, argValAsString.replace(/[^\s]/g, '*'));
		}
		return returnVal;
	}
}
...
export class RegExpReplacer implements Transformer {
	...
	transform(_stream: Stream, logRecord: LogRecord): LogRecord {
		...
		if (!shouldRedactMsg) {
			shouldRedactMsg = this.shouldRedact(logRecord.msg, this.#regExp, this.#replacer);
		}

		if (!shouldRedactMsg) {
			shouldRedactMeta = this.shouldRedact(logRecord.metadata, this.#regExp, this.#replacer);
		}

		if (shouldRedactMsg || shouldRedactMeta) {
			return new ObfuscatedViaRegExpLogRecord(logRecord, this.#regExp, this.#replacer);
		}
		return logRecord;
	}
	...
	constructor(logRecord: LogRecord, regExp: RegExp, replacer: Replacer) {
		...
		for (let i = 0; i < this.#metadata.length; i++) {
			if (typeof this.#metadata[i] === 'string') {
				this.#metadata[i] = (this.#metadata[i] as string).replace(regExp, replacer);
			} else {
