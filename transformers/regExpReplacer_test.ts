...
test({
	name: 'Errors are preserved when transforming into ObfuscatedPropertyLogRecord',
	fn() {
		...
		assertEquals((newLr.msg as Error).stack?.slice(0, 50), err.stack?.slice(0, 50));
		assertEquals((newLr.metadata[1] as Error).stack?.slice(0, 50), err2.stack?.slice(0, 50));
	},
});
...
test({
	...
	fn() {
		...
		const newLr2 = new RegExpReplacer(/[s]{2}.*\d{4}/).transform(noopStream, lr);
...
test({
	...
	fn() {
		...
		const newLr = new RegExpReplacer(/Message (\d{4})/).transform(noopStream, lr);
		assertEquals(newLr.msg, 'Log Message ****, hello world');

		const newLr2 = new RegExpReplacer(/Log ([a-zA-Z]+)/).transform(noopStream, lr);
		assertEquals(newLr2.msg, 'Log ******* 1234, hello world');

		const newLr3 = new RegExpReplacer(/Log ([a-zA-Z]+)\s\d(\d{2})\d/).transform(noopStream, lr);
...
test({
	...
	fn() {
		...
		const newLr = new RegExpReplacer(/123/).transform(noopStream, lr);
		assertEquals(newLr.metadata, [{ a: 'hello ***4', b: { c: '***4, there' } }, { d: '£76.22' }]);

		const newLr2 = new RegExpReplacer(/[o].*\d{4}/).transform(noopStream, lr);
		assertEquals(newLr2.metadata, [{ a: 'hell* ****', b: { c: '1234, there' } }, { d: '£76.22' }]);

		const newLr3 = new RegExpReplacer(/£([\d]+\.[\d]{2})/).transform(noopStream, lr);
		assertEquals(newLr3.metadata, [{ a: 'hello 1234', b: { c: '1234, there' } }, { d: '£**.**' }]);
	},
});
...
test({
	...
	fn() {
		...
		const newLr = new RegExpReplacer(/\d{2}-\d{2}-\d{4}/, (match) => match.replace(/\d/g, '*'))
...
test({
	...
	fn() {
		...
		const newLr = new RegExpReplacer(/\d{2}-\d{2}-\d{4}/, nonWhitespaceReplacer).transform(
			noopStream,
			lr,
		);
		assertEquals(newLr.msg, 'Date of birth: **********');

		const newLr2 = new RegExpReplacer(/Date of birth/, nonWhitespaceReplacer).transform(
			noopStream,
			lr,
		);
...
test({
	...
	fn() {
		...
		const newLr = new RegExpReplacer(/A(.*)Z/, nonWhitespaceReplacer).transform(noopStream, lr);
