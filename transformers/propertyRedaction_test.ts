...
test({
	...
	fn() {
		const lr = {
			msg: null,
			metadata: [{ a: 6, b: true }, 'The metadata', { c: 'hello', d: 'world' }, 1],
			dateTime: new Date('2020-06-17T03:24:00'),
			level: Level.Debug,
			logger: 'default',
		};
		...
		const newLrC = new PropertyRedaction('c').transform(noopStream, lr);
		const newLrD = new PropertyRedaction('d').transform(noopStream, lr);
		assertEquals(newLrA.metadata, [{ a: REDACTED, b: true }, 'The metadata', {
			c: 'hello',
			d: 'world',
		}, 1]);
		assertEquals(newLrB.metadata, [{ a: 6, b: REDACTED }, 'The metadata', {
			c: 'hello',
			d: 'world',
		}, 1]);
		assertEquals(newLrC.metadata, [
			{ a: 6, b: true },
			'The metadata',
			{ c: REDACTED, d: 'world' },
			1,
		]);
		assertEquals(newLrD.metadata, [
			{ a: 6, b: true },
			'The metadata',
			{ c: 'hello', d: REDACTED },
			1,
...
test({
	...
	fn() {
		...
		const newLrC = new PropertyRedaction('c').transform(noopStream, lr);
		const newLrD = new PropertyRedaction('d').transform(noopStream, lr);
		assertEquals(newLrA.metadata, [{ a: REDACTED, e: true }, { a: REDACTED, e: true }]);
		assertEquals(newLrB.metadata, [{ a: { b: REDACTED }, e: true }, {
			a: { b: REDACTED },
			e: true,
		}]);
		assertEquals(newLrC.metadata, [{ a: { b: { c: REDACTED } }, e: true }, {
			a: { b: { c: REDACTED } },
			e: true,
		}]);
		assertEquals(newLrD.metadata, [{ a: { b: { c: { d: REDACTED } } }, e: true }, {
			a: { b: { c: { d: REDACTED } } },
			e: true,
		}]);
	},
